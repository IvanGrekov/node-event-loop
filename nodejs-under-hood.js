...
// NOTE: Queues of Microtasks in NodeJS (Task queue takes one by one):
...
// Until we not empty `Next tick queue`, we can't start taking callbacks from `Promises and queueMicrotasks`

// Until we not empty `Promises and queueMicrotasks`, we can't start taking callbacks from `Next tick queue`
